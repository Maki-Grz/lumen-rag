...
async fn ingest(state: web::Data<AppState>, req: web::Json<IngestRequest>) -> impl Responder {
    println!("Ingesting text to MongoDB...");
...
async fn ask(state: web::Data<AppState>, req: web::Json<QuestionRequest>) -> impl Responder {
    ...
    if passages.is_empty() {
        return HttpResponse::Ok().body("Aucun résultat trouvé dans MongoDB.");
    }
...
async fn main() -> Result<()> {
    ...
    let mongo_uri = std::env::var("COSMOS_URI")
        .or_else(|_| std::env::var("MONGODB_URI"))
        .unwrap_or_else(|_| "mongodb://localhost:27017".to_string());

    let db_name = std::env::var("DATABASE").unwrap_or_else(|_| "lumen_db".to_string());
    let collection_name =
        std::env::var("COLLECTION").unwrap_or_else(|_| "knowledge_base".to_string());
    ...
    let store = MongoStore::new(client, db_name, collection_name);
    ...
    println!("MongoDB Server running on 0.0.0.0:8080");
    HttpServer::new(move || {
        App::new()
